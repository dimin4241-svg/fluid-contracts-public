#!/usr/bin/env python3
import concurrent.futures, datetime, json, requests, time
...
def rpc_call(url,address):
    ...
    r=requests.post(url,json=p,timeout=25,headers={'User-Agent':'fluid-fsl-automation/20260807'})
    r.raise_for_status(); b=r.json(); res=b.get('result') if isinstance(b,dict) else None
    if not isinstance(res,str) or not res.startswith('0x'): raise RuntimeError(str(b))
    return res

def one(t):
    network,chain,address,url=t
    row={'network':network,'chain_id':chain,'address':address,'canonical_routescan_url':url,'routescan_attempts':[],'fallback_attempts':[]}
    result=None; source=None
    for a in range(3):
        try:
            r=requests.get(url,timeout=25,headers={'User-Agent':'fluid-fsl-automation/20260807'})
            b=r.json() if r.content else None
            row['routescan_attempts'].append({'attempt':a+1,'status':r.status_code,'body':b})
            r.raise_for_status(); res=b.get('result') if isinstance(b,dict) else None
            if isinstance(res,str) and res.startswith('0x'):
                result=res; source='routescan'; break
        except Exception as e:
            row['routescan_attempts'].append({'attempt':a+1,'error':repr(e)})
        time.sleep(a+1)
    if result is None:
        for rpc in RPCS[chain]:
            try:
                result=rpc_call(rpc,address); source=rpc
                row['fallback_attempts'].append({'rpc':rpc,'result':result}); break
            except Exception as e:
                row['fallback_attempts'].append({'rpc':rpc,'error':repr(e)})
    if result is None:
        row['error']='No confirmed eth_call result'; return row
    raw=dec32(result)
    row.update({'source':source,'result':result,'raw_int32':raw,'annual_percent':raw/10000})
    if raw!=0:
        confirmations=[]; ok=False
        for rpc in RPCS[chain]:
            if rpc==source: continue
            try:
                r2=rpc_call(rpc,address); raw2=dec32(r2); match=(r2.lower()==result.lower() and raw2==raw)
                confirmations.append({'rpc':rpc,'result':r2,'raw_int32':raw2,'match':match})
                if match: ok=True; break
            except Exception as e:
                confirmations.append({'rpc':rpc,'error':repr(e)})
        row['independent_confirmations']=confirmations; row['nonzero_confirmed']=ok
        if not ok: row['error']='Nonzero not independently confirmed'
    return row
...
nonzero=[r for r in rows if r.get('raw_int32') not in (None,0) and r.get('nonzero_confirmed') is True and 'error' not in r]
...
print('FSL_RESULT_JSON='+json.dumps(out,separators=(',',':')))
