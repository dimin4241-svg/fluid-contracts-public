...
def request_json(url, method="GET", body=None, timeout=25):
    data = None if body is None else json.dumps(body).encode()
    req = urllib.request.Request(url, data=data, method=method, headers={"User-Agent":"fsl-watch/1.0","Content-Type":"application/json"})
    with urllib.request.urlopen(req, timeout=timeout) as r:
        return json.loads(r.read().decode())

def decode_int32(result):
...
def rpc(chain,address):
    ...
    errors=[]
    for url in FALLBACKS.get(chain,[]):
        try:
            p=request_json(url,"POST",body)
            r=p.get("result")
            if isinstance(r,str) and r.startswith("0x"):
                return r,url
            errors.append(url+": "+repr(p))
        except Exception as e:
...
def main():
    ...
    for network,chain,address,url in TARGETS:
        try:
            result,source=routescan(url)
        except Exception as e1:
            try:
                result,source=rpc(chain,address)
            except Exception as e2:
                failures.append({"network":network,"chain_id":chain,"address":address,"canonical_url":url,"routescan_error":str(e1),"fallback_error":str(e2)})
                continue
        raw=decode_int32(result)
        results.append({"network":network,"chain_id":chain,"address":address,"canonical_url":url,"source":source,"result":result,"raw_int32":raw,"annual_percent":raw/10000.0})
    out={"checked_at_utc":checked_at,"target_count":len(TARGETS),"success_count":len(results),"failure_count":len(failures),"nonzero":[x for x in results if x["raw_int32"] != 0],"results":results,"failures":failures}
    print("FSL_CHECK_JSON_START")
    print(json.dumps(out,indent=2))
    print("FSL_CHECK_JSON_END")
